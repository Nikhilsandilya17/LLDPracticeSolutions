...
#### User Profiles:
- Each account should have a profile with their information, such as profile picture, bio, and interests.
...
1. The **User** class represents a account in the social networking system, containing properties such as ID, name, email, password, profile picture, bio, list of friends, and list of posts.
2. The **Post** class represents a post created by a account, containing properties such as ID, account ID, content, image URLs, video URLs, timestamp, likes, and comments.
3. The **Comment** class represents a comment made by a account on a post, containing properties such as ID, account ID, post ID, content, and timestamp.
4. The **Notification** class represents a notification generated for a account, containing properties such as ID, account ID, notification type, content, and timestamp.
...
7. The SocialNetworkingService class provides methods for account registration, login, profile updates, friend requests, post creation, newsfeed generation, likes, comments, and notifications.
